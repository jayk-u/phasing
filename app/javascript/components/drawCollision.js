const drawCollisionShapes = (game, graph, object, name = false) => {
  graph.clear();
  ...
  object.forEachTile((tile) => {
    ...
    if (!collisionGroup || collisionGroup.objects.length === 0) { return; }
      ...
      for (var i = 0; i < objects.length; i++)
      {
          ...
          if (object.rectangle) {
            if (name == "visible") {game.visibleWalls.create(objectX, objectY, "ground").setSize(object.width, object.height).setOffset(16, 16).visible = false;}
            else if (name == "hidden") {game.hiddenWalls.create(objectX, objectY, "ground").setSize(object.width, object.height).setOffset(16, 16).visible = false;}
            else game.platforms.create(objectX, objectY, "ground").setSize(object.width, object.height).setOffset(16, 16).visible = false;
