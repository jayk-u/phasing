...
const minigameRiddles = (game, end) => {
  ...
  if (character.x <= 80) {
    ...
    displayNote("Why did outlaws sleep on the ground after robbing a bank?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("What did the prisoner about to get hanged say when he got pardoned?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("What did the hangman get his wife for her birthday?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("How do hangmen keep up with current events?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why was the photographer arrested?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("What did the thief get for stealing the calendar?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why did the sherriff arrest the cook?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("A person was in jail.\nAll he had in his cell was a piano.\nYet, he managed to escape.\nHow?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("What kind of party do prisoners like best?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("What kind of bars won't keep a prisoner in jail?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why was the picture sent to jail?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why are people in jail the slowest talkers ever?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why did the prisoner take a shower before he broke out of jail?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("How are prisoners like astronauts?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why can't you keep a clock in jail?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why did the outlaw steal the deck of cards?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why did the outlaw go to the river?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("What do outlaws eat with their milk?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why did the man shoot the clock?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Tool of thief, toy of queen.\nAlways used to be unseen.\nSign of joy, sign of sorrow.\nGiving all likeness borrowed.\nWhat am I?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why did the outlaw go to the river?");
  } else if (character.x <= 620 && character.y <= 590) {
    displayNote("Why did the outlaw go to the river?");
